...
pool=`ETCDCTL_API=3 /pace/etcdget.py run --prefix | grep $myhost | grep pool | grep name | awk -F'name' '{print $2}' | cut -c 5- | cut -c -2` 
...
do
 ...
 if [ $? -eq 0 ];
 then
   echo offlining disk $l >> /root/hostlosttmp
