...
$("#Group").change(function (e) {
	if ($("#Group").val().length > 2) {
		$("#UnixAddgroup").prop("disabled", false);
	} else {
		$("#UnixAddgroup").prop("disabled", true);
	}
});
		...
		columns: [
			...
			},
			{
				...
				render: function (data, type, row) {
					...
					return (
						'<select class="select2 multiple  groupusers ' +
						row.name +
						" " +
						cls +
						' form-control"' +
			...
			},
			{
				...
				render: function (data, type, row) {
					...
					return (
						'<button onclick="selbtnclickedgroup(this)" id="btnsel' +
						row.name +
						'" ' +
			...
			},
			{
				...
				render: function (data, type, row) {
					...
					return (
						'<a class="UnixDelgroup ' +
						cls +
						'" val="groupname" href="javascript:agroupdel(\'' +
						row.name +
						"')\" >" +
