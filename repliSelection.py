...
def selectDG(allinfo,volname, vol,snap):
 volused=levelthis(vol,'B')
 snapused=levelthis(snap,'B')
 volumes=[]
 print('#############################')
 print(list(allinfo['volumes']))
 print('#############################')
 print(volused, snapused)
 print('#############################')
 for vol in volumes:
  if vol['name'] in volname:
   print('found the volume')
 exit()
...
if __name__=='__main__':
 ...
 selectDG(allinfo,'volname','96K','1K')
# disks = allinfo['disks']
